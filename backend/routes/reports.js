...
const storage = multer.diskStorage({
  destination: (req, file, cb) => {
    const uploadPath = process.env.RENDER ? '/opt/render/project/src/uploads' : path.join(__dirname, '../uploads');
    cb(null, uploadPath);
  },
...
router.get('/:id/download', verifyToken, (req, res) => {
  ...
  db.get('SELECT * FROM reports WHERE id = ? AND (user_id = ? OR id IN (SELECT report_id FROM shared_access WHERE shared_with_user_id = ?))',
    ...
    const filePath = process.env.RENDER ? path.join('/opt/render/project/src/uploads', report.filename) : path.join(__dirname, '../uploads', report.filename);
